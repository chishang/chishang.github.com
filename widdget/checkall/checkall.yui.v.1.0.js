YUI.namespace('Checkall');
YUI.add('trip-checkall', function (Y) {
/**
 * 表单组件-chckbox 全选
 ...
 */
   function Checkall() {
        Checkall.superclass.constructor.apply(this, arguments);
    };
    Y.extend(Checkall,Y.Base, {
        /**
		 * 初始化组件
		 * @function
		 * @private
        ...
        getbox: function (type) {
            ...
            for (var i = 0; i < that.nodelist.length; i++) {
                ...
                if (node) {
                    node.get("checked") ? checked_node.push(node) : unchecked_node.push(node);
                }
        ...
        invert: function () {
            ...
            for (var i = 0; i < that.nodelist.length; i++) {
                var node = that.nodelist[i];
                if (node) {
                    var flag = node.get("checked") ? true : false;
        ...
        _bind: function () {
            ...
            for (var i = 0; i < that.node.length; i++) {
                that.node[i].on('click', function (e) {
                    ...
                    flag = node.get("checked") ? true : false;
                    that.checkall(flag);
                });
            }
        ...
        disable: function (flag, all) {
            var that = this;
            for (var i = 0; i < that.node.length; i++) {
                that.node[i].set('disabled', flag);
            }
            if (all != undefined) {
                for (var i = 0; i < that.nodelist.length; i++) {
                    that.nodelist[i].set('disabled', flag);
                }
            }
        }
    });
